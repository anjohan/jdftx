...
class JDFTx(Calculator):
    def __init__(
        ...
        self.executable = replaceVariable(
            executable, "JDFTx"
        )  # Path to the jdftx executable (cpu or gpu)
        self.pseudoDir = replaceVariable(
            pseudoDir, "JDFTx_pseudo"
        )  # Path to the pseudopotentials folder

        if self.executable is None:
            raise Exception(
                "Specify path to jdftx in argument 'executable' or in environment variable 'JDFTx'."
            )
        if (self.pseudoDir is None) and (not (pseudoSet in pseudoSetMap)):
            raise Exception(
                "Specify pseudopotential path in argument 'pseudoDir' or in environment variable 'JDFTx_pseudo', or specify a valid 'pseudoSet'."
            )
        ...
        self.input = [("dump-name", "$VAR"), ("initial-state", "$VAR")]
    ...
    def __readForces(self, filename):
        ...
        for line in open(filename):
            if line.startswith("force "):
                ...
                forces[idx] = [
                    float(word) for word in tokens[2:5]
                ]  # tokens[2:5]: force components

        if len(forces) == 0:
            raise IOError("Error: Forces not found.")
        return (Hartree / Bohr) * scipy.array(forces)

    def __readStress(self, filename):
        return np.loadtxt(filename) * Hartree / Bohr ** 3
    ...
    def runJDFTx(self, inputfile):
        ...
        s = self.__readStress("%s/stress" % (self.runDir))
        self.Stress = np.array([s[0, 0], s[1, 1], s[2, 2], s[1, 2], s[0, 2], s[0, 1]])
        # self.Stress = s

        # Store results where file writers will look for them
    ...
    def constructInput(self, atoms):
        ...
        # Add ion info
        atomPos = [
            x / Bohr for x in list(atoms.get_positions())
        ]  # Also convert to bohr
        ...
        if not (self.pseudoDir is None):
            ...
            for atom in atomNames:
                if (
                    sum([x == atom for x in added]) == 0.0
                ):  # Add ion-species command if not already added
                    for filetype in self.pseudopotentials:
                        try:
                            shell(
                                "ls %s | grep %s.%s" % (self.pseudoDir, atom, filetype)
                            )
    ...
    def help(self, command=None):
        ...
        if command is None:
            print(
                "This is the help function for JDFTx-ASE interface. \
